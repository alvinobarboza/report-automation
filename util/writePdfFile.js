...
const getBodyData = (old, neW) => {
    ...
    const sumTotal = lastMonthAmount + amount;
    const totalToReport = sumTotal / 2;
    return {
        content: [
            { text: 'ANEXO 2 - Relatório de Assinantes', style: 'header' },
            {
                ...
                table: {
                    widths: ['*'],
                    body: [[{ text: 'RELATÓRIO BASE DE ASSINANTES' }]],
                },
            },
            {
                style: 'tableExample',
                table: {
                    ...
                    body: [
                        [
                            { text: 'Data', fillColor: '#bfbfbf' },
                            getDateAstertes(),
                        ],
                        [
                            { text: 'Licenciado', fillColor: '#bfbfbf' },
                            {
                                text: 'YOU CAST COMERCIO DE EQUIPAMENTOS ELETRONICOS LTDA',
                            },
                        ],
                        [
                            {
                            ...
                            {
                                text: `${getCurrentMonth()}_${getCurrentYear()}`,
                            },
                        ],
                        [
                            {
                                text: 'Número total de assinantes no início do mês',
                                fillColor: '#bfbfbf',
                            },
                            { text: lastMonthAmount },
                        ],
                        [
                            {
                                text: 'Número total de assinantes no final do mês',
                                fillColor: '#bfbfbf',
                            },
                            { text: amount },
                        ],
            ...
            },
            {
                ...
                table: {
                    ...
                    body: [
                        [
                            {
                                text: 'Provedores agregados',
                                fillColor: '#bfbfbf',
                            },
                            {
                                text: 'Número de assinantes',
                                fillColor: '#bfbfbf',
                            },
                        ],
            ...
            },
            {
                ...
                table: {
                    ...
                    body: [
                        [
                            {
                                text: 'Responsável pelo report:',
                                fillColor: '#bfbfbf',
                            },
                            {
                                text: 'Carlos Eduardo Salce/Alvino N. C. Barboza',
                            },
                        ],
                    ],
                },
...
const saveAstartesCustomers = (amount) => {
    ...
    const { year, month, day } = convertTimestampToYearMonthDay(
        new Date().getTime()
    );
    ...
    for (let i = 0; i < AstartesData.data.length; i++) {
        ...
        if (
            storedDate.month === month &&
            storedDate.year === year &&
            storedDate.day === day
        ) {
...
const writePdfFile = (old, neW, insertFilenameToFilenames, getPath) => {
    try {
        ...
        const filename = getPath(
            `Relatório Base de Assinantes - ${getCurrentMonth()}_${getCurrentYear()}.pdf`
        );
